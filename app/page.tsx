...
export default function Home() {
  return (
    <div className="text-[#1c1c1c]">
          ...
          <section key={cat.id} id={cat.id} className={`${cat.bg} py-14 md:py-24 px-4 md:px-8`}>
            <div className="max-w-7xl mx-auto">
              <div className="flex flex-col md:flex-row md:items-end md:justify-between mb-8 md:mb-12 gap-3 md:gap-4">
                ...
                <p className="text-[#888] text-sm leading-loose max-w-xs md:text-right">{cat.desc}</p>
              </div>
      ...
      {/* ── PORTFOLIO ────────────────────────────────────────────── */}
      <section id="portfolio" className="hidden bg-[#1a1a1a] py-16 md:py-28 px-4 md:px-8">
        <div className="max-w-7xl mx-auto">
            ...
            {projects.map((p, idx) => {
              ...
              return (
                <Link
                  ...
                  className={["group relative overflow-hidden bg-[#2a2a2a]", !isLast && p.colSpan === 2 ? "col-span-2" : "", !isLast && p.rowSpan === 2 ? "md:row-span-2" : ""].join(" ")}
      ...
      {/* ── ABOUT ────────────────────────────────────────────────── */}
      <section id="about" className="flex flex-col md:flex-row min-h-screen bg-[#edeae4]">
        <div className="relative w-full md:w-1/2 min-h-[55vw] md:min-h-0">
          <Image src={imgSrc("칸톤 홍보관", "L7R06048.JPG")} alt="About CNS Design" fill loading="lazy" className="object-cover" sizes="50vw" />
        </div>
        <div className="w-full md:w-1/2 flex items-center justify-center bg-[#edeae4] px-6 md:px-12 py-14 md:py-24">
          <div className="max-w-105 w-full">
            <p className="text-[0.65rem] tracking-[0.5em] uppercase text-[#a08060] mb-5">About Us</p>
            <h2 className="text-2xl md:text-3xl font-extralight mb-8 md:mb-10 leading-relaxed">씨앤에스디자인</h2>
            <p className="text-[#666] leading-loose mb-6 text-[0.95rem]">씨앤에스디자인은 상업 공간, 호텔, 카페, 피트니스, 사무실 등 다양한 분야의 인테리어를 전문으로 합니다.</p>
            <p className="text-[#666] leading-loose text-[0.95rem]">클라이언트의 비전을 공간으로 실현하고, 사람과 공간이 만들어내는 이야기를 디자인합니다.</p>
            <div className="mt-10 md:mt-12 flex gap-10 md:gap-12 border-t border-black/8 pt-8">
              <div>
      ...
      {/* ── CONTACT ──────────────────────────────────────────────── */}
      <section id="contact" className="bg-[#1a1a1a] text-white py-16 md:py-28 px-4 md:px-8">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="mt-12 md:mt-20 pt-8 border-t border-white/10 flex items-center justify-between">
            <div className="flex flex-col gap-1">
              <p className="text-white/20 text-xs tracking-wider">© 2026 CNS Design. All rights reserved.</p>
